...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(dtype=tf.uint8, shape=[None, state_shape[0], state_shape[1], state_shape[2] * self.config.state_history])
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_collection = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope=q_scope)
        target_collection = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        op = []
        for q, t in zip(q_collection, target_collection):
            op.append(tf.assign(t, q))
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_sample = self.r + self.config.gamma * tf.reduce_max(target_q, axis=1) * \
        (1 - tf.cast(self.done_mask, dtype=tf.float32))
        q_new = tf.reduce_sum(q * tf.one_hot(self.a, num_actions), axis=1)
        self.loss = tf.reduce_mean(tf.square(q_sample - q_new))
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        if self.config.grad_clip:
            ...
            for gv in grads_and_vars:
                clipped_grads_and_vars.append((tf.clip_by_norm(gv[0], self.config.clip_val), gv[1]))
